...
def run_migrations_online() -> None:
    ...
    def include_object(object, name, type_, reflected, compare_to):
        if type_ == "table" and object.schema in ("tiger", "topology", "tiger_data"):
            return False
        if name in ("spatial_ref_sys",):
            return False
        # Filter out other common PostGIS tables that shouldn't be dropped
        if name and name.startswith("tiger."):
            return False
        return True
