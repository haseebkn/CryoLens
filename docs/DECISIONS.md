...
* **Consequences:** Measured suppression is $44.5\times$ overall and $105\times$ over open water, taking detection density from $50.5$ to $1.13$ per $1000\text{ km}^2$ overall, and to $0.28$ over open water. The published ledger shows the minimum-size stage alone accounts for 97.3 percent of removals. The recall cost is **not measured**, because no iceberg ground truth exists for these scenes; raising the minimum size threshold necessarily discards small icebergs. This trade-off is stated in `docs/LIMITATIONS.md` rather than buried.
